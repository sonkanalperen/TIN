...
import { NewsArticle, ThemeType, ThemeMode } from './types';
...
const AppContent: React.FC = () => {
  ...
  const switchView = useCallback((v: string, category?: string) => {
    const viewMap: Record<string, typeof view> = {
      'newspaper': 'gazete', 'home': 'sohbet', 'user-space': 'profil', 'koc': 'koc', 'core': 'core', 'haber': 'haber'
    };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setChatMessages(prev => [...prev, { role: 'model', text: "Bağlantı hatası." }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col relative transition-all duration-700">
      ...
      <Header setView={switchView} activeView={view} theme={theme} mode={mode} setTheme={setTheme} setMode={setMode} isAuthenticated={isAuthenticated} />
